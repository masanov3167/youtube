...
const Video = () => {
	const { id } = useParams();
	const { mode, channels,getId } = React.useContext(Context);
 ...
 if(videos.isFetched){
    fetch(`https://jsonplaceholder.typicode.com/photos?albumId=${videos.data[0].albumId}`)
        .then((res) => res.json())
        .then((data) =>
          setNext({
            isFetched: true,
...
return (
		<>
			...
			<main
				...
				{videos.isFetched ? (
					videos.data.map((e) => (
						<div className='active-video' key={e.id}>
							<img src={e.url} alt='chanel img' />
							<p style={mode ? { color: 'white' } : { color: '#19202c' }}>
								{e.title}
							</p>
							<div className='active-video-stats d-flex'>
								<time style={mode ? { color: 'white' } : { color: '#19202c' }}>123k wiews</time>

								<ol className='d-flex'>
									<li style={mode ? {backgroundColor: '#ffffff1a'} : {backgroundColor: '#ebebeb4d'}} >
										<svg
											width='14'
											height='13'
											viewBox='0 0 14 13'
										...
										<time style={mode ? { color: 'white' } : { color: '#19202c' }}>123k</time>
									</li>
									...
									<li style={mode ? {backgroundColor: '#ffffff1a'} : {backgroundColor: '#ebebeb4d'}}>
										...
										<time style={mode ? { color: 'white' } : { color: '#19202c' }}>435k</time>
									</li>
									<li style={mode ? {backgroundColor: '#ffffff1a'} : {backgroundColor: '#ebebeb4d'}}>
										<svg
											width='14'
                    ...
                    <time style={mode ? { color: 'white' } : { color: '#19202c' }}>Share</time>
									</li>
                  <li style={mode ? { color: 'white' } : { color: '#19202c' }}>•••</li>
								</ol>
							</div>
              <hr />

              <div className='active-video-bottom d-flex' >
				  <NavLink className='d-flex' onClick={getId}  to='/channel' data-channel-id={e.albumId - 1}>
				  			<img src={channels[e.albumId - 1].img} alt="channel img" data-channel-id={e.albumId - 1} />
							<h2 style={mode ? { color: 'white' } : { color: '#19202c' }} data-channel-id={e.albumId - 1}>{channels[e.albumId - 1].title}</h2>
                 </NavLink>

                  <NavLink className='subscribe' onClick={getId}  to='/channel' data-channel-id={e.albumId - 1} >Subscribe</NavLink>
              </div>
						</div>
					))
				) : (
            ...
            <div className='next-video-wrapper'>
              <div className='d-flex' style={{marginBottom:'20px'}} >
                <h2 className='next-video-title' style={mode ? { color: 'white' } : { color: '#19202c' }}>Next</h2>
                <div className='d-flex'>
                  <p className='next-video-autoplay' style={mode ? { color: 'white' } : { color: '#19202c' }}>autoplay</p>
                  <input className='next-video-input' type="checkbox" />
                </div>
              </div>
                {next.isFetched ? (
                  next.data.filter(a => a.id !== Number(id)).map((e) => (
                    <NavLink to={`/video/${e.id}`} key={e.id} className="next-video">
                      <img src={e.url} alt="next posts" />
                      <p style={mode ? {color: 'white'} : {color:'#000000'}}>{e.title}</p>
                      <div className='d-flex' style={{margin:'12px 0'}} >
                          <time style={mode ? {color: 'white', opacity:'0.4' } : {color:'#C2C2C2'}}>11k views</time>
                          <h3 style={mode ? {color: 'white'} : {color:'#19202C'}}>{channels[videos.data[0].albumId - 1].title}</h3>
                      </div>
                  </NavLink>
                  ))
                ) : (
